    ...
    <main className="jobPage" style={{opacity:pageReady?1:0,visibility:pageReady?"visible":"hidden",transition:"opacity 220ms ease-out"}}>
      ...
      <aside className="jobSidebar">
        ...
        <div className="jobProfile">
          <label className="profilePhotoControl" title="Subir o cambiar foto">
            <input type="file" accept="image/*" onChange={(e) => { const file=e.target.files?.[0]; if(!file)return; setProfilePhoto(URL.createObjectURL(file)); notify("Foto de perfil actualizada."); }}/>
            {profilePhoto ? <img src={profilePhoto} alt="Foto de perfil" className="jobProfilePhoto"/> : <div className="jobAvatar">MR</div>}
          </label>
          <div><strong>Hola, Mariana</strong><span>Usuario verificado ✓</span><small>Perfil laboral WorkCerca</small></div>
        </div>
        <nav className="masterJobNav">
          ...
          <button onClick={() => window.location.href="/mi-workcerca#presupuestos"}>▧ <span>Presupuestos</span></button>
          <button onClick={() => window.location.href="/mi-workcerca#mensajes"}>▱ <span>Mensajes</span></button>
          <button onClick={() => window.location.href="/mi-workcerca#videollamadas"}>▣ <span>Videollamadas</span></button>
          <button onClick={() => window.location.href="/mi-workcerca#agenda"}>□ <span>Agenda</span></button>
          <button onClick={() => window.location.href="/mi-workcerca#proyectos"}>▣ <span>Proyectos</span></button>
        ...
        <div className="jobTrust"><b>✦ IA WorkCerca</b><p>Te ayuda a construir un CV claro y conectarlo con oportunidades reales sin inventar experiencia ni capacidades.</p></div>
      </aside>
